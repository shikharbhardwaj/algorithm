...
template 
<typename attrib, class graph_property, 
 class edge_property, typename enable = void> class graph;

template <typename attrib, class graph_property, class edge_property>
class graph<attrib, graph_property, edge_property,
    typename std::enable_if <std::is_same <graph_property,
        graph_properties::list>::value>::type> {
...
public:
    ...
    void add_edge(typename std::enable_if<std::is_same<edge_property,
            edge_properties::undirected>::value, attr>::type id1, attr id2) {
    ...
    template <typename attr>
    void add_edge(typename std::enable_if<std::is_same<edge_property,
            edge_properties::directed>::value, attr>::type id1, attr id2) {
...
class graph<attrib, graph_property,
        typename std::enable_if <std::is_same <graph_property, graph_properties::matrix>::value>::type> {
