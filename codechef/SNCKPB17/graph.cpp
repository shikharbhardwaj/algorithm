...
void dfs(int src, vector<set<pii>>& graph, vector<bool>& visited){
	++cur_time;
	visited[src] = true;
	times[src] = cur_time;
	for(auto e : graph[src]){
		if(!visited[e.second]){
			dfs(e.second, graph, visited);
		} else{
			if(times[src] < times[e.second]){
				deg_back_edges[min(graph[e.second].size(), graph[src].size())]++;
			}
		}
	}
}
...
int main(){
	...
	TEST{
		...
		for(auto e : edge_set){
			...
			graph[u].insert({max_deg - min_deg, v});
			graph[v].insert({max_deg - min_deg, u});
		}
