...
#include <iostream>
#include <vector>
template <typename T> int max_pow(T n, T m) {
    int cnt = 0;
    while (n % m == 0) {
        n /= m;
        cnt++;
...
template <typename T> int max_pow_fact(T n, T m) {
    int accum = 0;
    while (n > 1) {
        accum += n / m;
        n /= m;
...
template <typename T> int max_pow_C(T n1, T n2, T m) {
    int accum = 0;
    while (n1 > 1) {
        accum += n1 / m;
        accum -= n2 / m;
...
void apply_once(int id) {
    ...
    if (cur.update) {
        ...
        if (cur.update == 2) {
            cur.pow2 = cur.pow5 = 0;
            cur.pow2 += max_pow_C(cur.right - cur.i + 1, cur.left - cur.i, 2);
            cur.pow5 += max_pow_C(cur.right - cur.i + 1, cur.left - cur.i, 5);
        }
        cur.pow2 += add2;
        cur.pow5 += add5;
    }
...
void update_range_mul(int id, int left, int right, int multi2, int multi5) {
...
void update_range_repl(int id, int left, int right, int L, int multi2,
                       int multi5) {
    auto &cur = segtree[id];
    apply_update(id);
    if (cur.left > right || cur.right < left) {
...
void display(int i, int level) {
    ...
    indent(5 * level);
    new_level(3);
    std::cout << "\n[" << segtree[i].left << ", " << segtree[i].right
              << "] -> (" << segtree[i].pow2 << ", " << segtree[i].pow5
              << ") :: " << segtree[i].update << " | " << segtree[i].x << " ~ "
              << segtree[i].i;
    display(2 * i, level + 1);
    display(2 * i + 1, level + 1);
}
int main() {
