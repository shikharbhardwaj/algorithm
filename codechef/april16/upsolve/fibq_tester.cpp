...
void CombineIntervalInfo(const struct IntervalInfo &left,
                         const struct IntervalInfo &right,
                         struct IntervalInfo *combined) {
    ...
    combined->sfib = ((long long)left.sfib * (long long)right.sfibp1 +
                      (long long)left.sfibm1 * (long long)right.sfib) %
                     MOD;
    combined->sfibm1 = ((long long)left.sfib * (long long)right.sfib +
                        (long long)left.sfibm1 * (long long)right.sfibm1) %
                       MOD;
    combined->sfibp1 = ((long long)left.sfibp1 * (long long)right.sfibp1 +
                        (long long)left.sfib * (long long)right.sfib) %
                       MOD;
    combined->sfib = ((long long)combined->sfib + (long long)left.sfib +
                      (long long)right.sfib) %
                     MOD;
    combined->sfibm1 = ((long long)combined->sfibm1 + (long long)left.sfibm1 +
                        (long long)right.sfibm1) %
                       MOD;
    combined->sfibp1 = ((long long)combined->sfibp1 + (long long)left.sfibp1 +
                        (long long)right.sfibp1) %
                       MOD;
}
...
int Fibo(int n) {
    ...
    if (fibo_cache_it != fibo_cache.end())
        return fibo_cache_it->second;
    int f, right, left;
    right = (n >> 1);
    left = n - right;
    f = ((long long)Fibo(left) * (long long)Fibo(right + 1) +
         (long long)Fibo(left - 1) * (long long)Fibo(right)) %
        MOD;
...
void InitSegTree() {
    ...
    for (int i = N; i < NMAX; i++) {
        segtree[NMAX + i].sfib = segtree[NMAX + i].sfibm1 =
            segtree[NMAX + i].sfibp1 = 0;
        segtree[NMAX + i].l = segtree[NMAX + i].r = i + 1;
    }
...
void QuerySegTree(int node) {
    ...
    int left = node << 1;
    int right = left + 1;
    if (QA <= segtree[left].r)
        QuerySegTree(left);
    if (QB >= segtree[right].l)
        QuerySegTree(right);
}
